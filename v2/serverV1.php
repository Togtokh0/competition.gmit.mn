<?php
  include('db.php');
    ...
    if (isset($_POST['pay_user'])) {
      ...
      $sql = "UPDATE web_data SET bc_color ='$color', bc_image ='$img', page ='$page'  WHERE id=1";
    	mysqli_query($db, $sql);
    	header("location: ed_site.php");
    }
...
function isAdmin()
{
  if (isset($_SESSION['user_data']) && $_SESSION['user_data']['user_role'] == 'admin' || isset($_SESSION['user_data']) && $_SESSION['user_data']['user_role'] == 'developer'|| isset($_SESSION['user_data']) && $_SESSION['user_data']['user_role'] == 'god' ) {
    return true;
  }else{
    return false;
  }
...
function isGOD()
{
  if (isset($_SESSION['user_data']) && $_SESSION['user_data']['user_role'] == 'god' ) {
    return true;
  }else{
    return false;
  }
...
function isMOD()
{
  if (isset($_SESSION['user_data']) && $_SESSION['user_data']['user_role'] == 'moderator' ) {
    return true;
  }else{
    return false;
  }
...
$count_suppliers= mysqli_query($db, "SELECT * FROM members  ");
...
$about_lan = array(
  'about' => 'Үйлдвэртэй хамтын ажиллагааны платформ',
  'about_in' => 'Үйлдвэртэй, компаниудын хувьд тавигдсан асуудлыг шийдвэрлэхэд оролцож, өөрийгөө сорих хүсэлтэй их дээд сургууль, хүрээлэнгийн нэгж , судалгааны багийн хувьд энэхүү платформ нээлттэй.Бүртгүүлээд бидэнтэй хамтран ажиллахыг хүсье.' ,
  ...
  'contact_add'=>'GMIT campus, 2nd khoroo, Nalaikh district, Ulaanbaatar ',
...
$about_lan_mn = array(
  ...
  'contact_add'=>'Улаанбаатар, Налайх дүүрэг, 2-р хороо, МГТИС-ийн хотхон',
